...
write_delim(iucn_arthropods_species_gbif, "../results/iucn_arthropods_species_gbif.tsv", delim="\t")
...
gnr_species <- gnr_resolve(endemic_species$scientificName)
gnr_species_gbif <- gnr_resolve(endemic_species$scientificName, data_source_ids=11)

write_delim(gnr_species, "../results/gnr_species_names.tsv", delim="\t")
# Get GBIF ids

res <- get_gbifid(endemic_species$scientificName,ask=F)
# Total: 343
# Found: 306
...
endemic_species_tax <- endemic_species |>
    left_join(classification_s_d, by=c("gbif"="gbif"))
