...
order_sites <- locations_shp %>%
    mutate(lon=st_coordinates(locations_shp)[,1],
           lat=st_coordinates(locations_shp)[,2]) %>%
    distinct(Order,lon,lat) %>%
    group_by(Order) %>%
    summarise(sites=n())
...
order_total_l <- list(order_taxa,order_sites,order_locations,order_iucn,order_paca)
...
total_data <- data.frame(Order="total",
                         taxa=length(unique(endemic_species$subspeciesname)),
...
kable(total_summary, "latex") %>% landscape(margin = NULL)
```
