...
locations_spatial_d <- locations_spatial %>% 
    ...
    mutate(elevation_bin=cut(elevation, 
                             breaks=seq.int(from=0, to=2500, by=100),
                             dig.lab = 5 ))
...
```{r, warning=FALSE, message=FALSE, echo=T}
g_humb <- ggplot() +
    geom_col(locations_spatial_d_b_i, mapping=aes(x=n_species, y= elevation_bin, fill=iucn))

ggsave("../plots/elevation_humb.png", plot=g_humb, device="png")
...
```{r, warning=FALSE, message=FALSE, echo=T}
g_elev_h <- ggplot() +
    geom_histogram(locations_spatial_d,mapping=aes(y=elevation, fill=iucn),bins=100)

ggsave("../plots/elevation_hist.png", plot=g_elev_h, device="png")
